...
export default function MyLog() {
  ...
  const handleIssueInfo = (el) => {
    ...
    const pushThumbsUp = (el) => {
      if(el.hostId===id) {
        axios.post(`${process.env.REACT_APP_SERVER_API}/mypage`, {
          postId: el.postId,
          giverId: id,
          recevierId: el.guestId
        })
        .then(()=>{
          axios.get(`${process.env.REACT_APP_SERVER_API}/user/auth`, {
            withCredentials: true,
    ...
    const deleteThumbsUp = (el) => {
      if(el.hostId===id) {
        axios.delete(`${process.env.REACT_APP_SERVER_API}/mypage`, {
          data : {
            postId: el.postId,
            giverId: id,
            recevierId: el.hostId
  ...
  <div className='mylog-container'>
    ...
    <th className='date'>일시</th>
        ...
        {isLoading? (
          ...
          dummyData.map((el, i) => {
            return <DataRow el={el} key={i} handleIssueInfo={handleIssueInfo} setIssue={setIssue} deleteThumbsUp={deleteThumbsUp} pushThumbsUp={pushThumbsUp}/>;
          })
        ))}
      </table>
        </div>
...
const DataRow = ({ el, handleIssueInfo, setIssue, deleteThumbsUp, pushThumbsUp }) => {
  return (
    <tr className={el.isMatched===2 ? 'canceled' : ''}>
      <td className='date'><Link to={`/view/${el.postId}`} style={{ color: 'inherit', textDecoration: 'inherit' }}><span>{el.reserved_at.slice(0,10) + ' ' + el.reserved_at.slice(11,16)}</span></Link></td>
      <td className='location'><Link to={`/view/${el.postId}`} style={{ color: 'inherit', textDecoration: 'inherit' }}><span>{el.location.place_name}</span></Link></td>
      <td className='hostGuest'><Link to={`/view/${el.postId}`} style={{ color: 'inherit', textDecoration: 'inherit' }}><span>{el.guestNickname ? `${el.hostNickname}/${el.guestNickname}` : el.hostNickname}</span></Link></td>
      <td className='match'><Link to={`/view/${el.postId}`} style={{ color: 'inherit', textDecoration: 'inherit' }}><span>{el.isMatched===0 ? '모집중' : '마감'}</span></Link></td>
      <td className='etc'>
      {el.isMatched===2 ? (
      ...
      : (el.isMatched===1 ?(
        ...
        {el.isThumbsup ? <FontAwesomeIcon icon={faThumbsUp} className='btn-thumbsup' onClick={()=>{deleteThumbsUp(el)}}/> : <FontAwesomeIcon icon={unFaThumbsUp} className='btn-thumbsup' onClick={()=>{pushThumbsUp(el)}}/>}
